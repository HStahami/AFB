...
import { MessageCircle, Send, User, RefreshCw, CheckCheck } from "lucide-react";
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const sentMsg = await messagesApi.send({
        thread_id: threadId,
        recipient_user_id: activeThread.instructor_user_id,
        content: content,
      });
      ...
      <div style={{ marginBottom: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h1 style={{ fontSize: "1.8rem", fontWeight: "700", color: "#fff" }} className="gradient-text">
            Instructor Chat & Messages
          </h1>
          <p style={{ color: "#8892b0", fontSize: "0.95rem" }}>
            Directly communicate with your assigned Arabic & Quranic Ustadh
          </p>
        </div>
        <button
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: "1.5rem", minHeight: "550px" }}>
        ...
        <div className="glass-panel" style={{ padding: "1.2rem", display: "flex", flexDirection: "column" }}>
          <h3 style={{ color: "#fff", fontSize: "1.1rem", marginBottom: "1rem", display: "flex", alignItems: "center", gap: "8px" }}>
            <MessageCircle size={18} color="var(--color-primary)" /> Conversations
          </h3>

          {loadingThreads ? (
            <p style={{ color: "#8892b0", textAlign: "center", margin: "auto" }}>Loading chats...</p>
          ) : threads.length === 0 ? (
            <p style={{ color: "#8892b0", textAlign: "center", margin: "auto", fontSize: "0.9rem" }}>
              No active instructor conversations assigned yet.
            </p>
          ) : (
          ...
          {activeThread ? (
            <>
              ...
              <div style={{ borderBottom: "1px solid rgba(255, 255, 255, 0.1)", paddingBottom: "1rem", marginBottom: "1rem", display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <div>
                  <h3 style={{ color: "#fff", fontSize: "1.1rem", margin: 0 }}>
                    {activeThread.instructor_name || "Ustadh"}
                  </h3>
                  <span style={{ color: "var(--color-primary)", fontSize: "0.8rem" }}>Assigned Instructor</span>
                </div>
              </div>
              ...
              <div style={{ flex: 1, overflowY: "auto", paddingRight: "8px", display: "flex", flexDirection: "column", gap: "12px" }}>
                {loadingHistory ? (
                  <p style={{ color: "#8892b0", textAlign: "center", margin: "auto" }}>Loading messages...</p>
                ) : messages.length === 0 ? (
                  <div style={{ textAlign: "center", margin: "auto", color: "#8892b0" }}>
                    <MessageCircle size={36} style={{ marginBottom: "8px", opacity: 0.5 }} />
                    <p>No messages yet in this conversation.</p>
                    <p style={{ fontSize: "0.85rem" }}>Send a message below to ask questions about your coursework or lessons.</p>
                  </div>
                ) : (
                  messages.map((m, idx) => {
                    const isMe = m.sender_role === "student" || m.sender_user_id === str(user?.id || user?._id);
                    return (
                      <div
                        key={m._id || m.id || idx}
                        style={{
